...
class SchedulerGUI:
    ...
    def build_input_panel(self):
        ...
        ttk.Button(top_ctrl, text="Load File", command=self.load_from_file).pack(side='left', padx=5)
        ttk.Button(top_ctrl, text="Clear All", command=self.clear_all).pack(side='left', padx=5)
        ttk.Label(top_ctrl, text="(Double click any cell below to edit)").pack(side='left', padx=15)
    ...
    def load_from_file(self):
        filepath = filedialog.askopenfilename(title="Open Input File", filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")])
        if not filepath:
            return
        try:
            with open(filepath, 'r') as f:
                ...
                for i in range(n):
                    if idx + 3 < len(content):
                        pid = int(content[idx])
                        at = int(content[idx+1])
                        bt = int(content[idx+2])
                        prio = int(content[idx+3])
                        self.tree_input.insert('', 'end', values=(pid, at, bt, prio))
                        idx += 4
                    else:
                        break
                self.n_var.set(n)
    ...
    def build_controls(self):
        ...
        self.algo_cb = ttk.Combobox(frame, textvariable=self.algo_var, values=["FCFS", "SJF", "SRTF", "Priority_NP", "Priority_P", "RR", "HRRN", "LJF", "LRTF"], state="readonly")
